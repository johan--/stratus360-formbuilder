({
    onChange : function(component, event, helper){
        ...
        var day = component.get('v.IsDayPresent') ? component.find('InputDay').get('v.value') + '-' : '';
    ...
    setDayOpts : function(month, year=1, defaultDay=1) {
        ...
        var nDays;  var dayOpts = [];

        if(month[0] == ";") month = month.substring(1);
        ...
        if(month == ""){
            nDays = 0;
            if(!Number.isNaN(defaultDay)) dayOpts.push({class:'optionClass', label:String(defaultDay), value:String(defaultDay)});
        }
        ...
        for(var i = 1; i <= nDays; i++){
            var row = { class: "optionClass", label: String(i), value: String(i) };
            if(i == defaultDay) row.selected = "true";
            dayOpts.push(row);
        }
        dayOpts.unshift({ class: "optionClass", label: "", value: "" });
        return dayOpts;
    },
