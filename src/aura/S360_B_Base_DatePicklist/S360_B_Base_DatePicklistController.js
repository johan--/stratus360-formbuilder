({
    doInit : function(cmp, event, helper) {
        ...
        var defaultDay = cmp.get('v.DefaultDay') 
        ? (cmp.get("v.IsDisabled") 
            ? "" 
            : cmp.get('v.DefaultDay')) 
        : "";
        
        var defaultMonth = cmp.get('v.DefaultMonth') 
        ? (cmp.get("v.IsDisabled") 
            ? "" 
            : cmp.get('v.DefaultMonth')) 
        : "";
        
        var defaultYear = cmp.get('v.DefaultYear') 
        ? (cmp.get("v.IsDisabled") 
            ? "" 
            : cmp.get('v.DefaultYear')) 
        : "";

        var dayOpts = []; 
        var monthOpts = [];
        var yearOpts = [];
        // Disable day input if month isn't selected 
        // (it needs month to be set to display options)
        // Then set selected month to default value
        if(defaultMonth == "") {
            cmp.set('v.DayDisabled');
        }

        if(cmp.get("v.PicklistMonth") && cmp.get("v.PicklistMonth").length){
            var rawOpts = cmp.get("v.PicklistMonth").split(',');
            for(var i = 0; i < rawOpts.length; i++){
                monthOpts.push({
                    "class": "optionClass",
                    "label": String(rawOpts[i]).split("|")[1],
                    "value": String(rawOpts[i]).match(/(\w*)|/)[0],
                    "selected": String(defaultMonth) == String(rawOpts[i]).match(/(\w*)|/)[0]
                });
            }
            monthOpts.unshift({
                "class": "optionClass",
                "label": "",
                "value": "",
                "selected": String(defaultMonth=="")
            });
            for(var i = 0; i< rawOpts.length; i++){
                if(rawOpts[i].match(/(\w*)|/)[0] == cmp.get('v.DefaultMonth')){
                    cmp.set('v.monthToDisplay', rawOpts[i].split("|")[1]);
                    break;
                }
            }
            ...
            monthOpts = [
                { "class": "optionClass", "label": "", "value": "", "selected":(String(defaultMonth)=="")},
    ...
    changeMonth: function(cmp,evt,hlp){
        ...
        if(cmp.get('v.DefaultMonth') == '') cmp.set('v.DefaultDay', '');
    ...
    disabledChange: function(cmp,evt,hlp){    
        if(cmp.get('v.IsDisabled')){
            ...
            cmp.set('v.monthToDisplay', '');
        }
    },
    readOnlyChange: function(cmp,evt,hlp){
        if( typeof cmp.get("v.DefaultDay") == "undefined") cmp.set('v.DefaultDay', '');
        if( typeof cmp.get("v.DefaultMonth") == "undefined") cmp.set('v.DefaultMonth', '');
        if( typeof cmp.get("v.DefaultYear") == "undefined") cmp.set('v.DefaultYear', '');
    },
